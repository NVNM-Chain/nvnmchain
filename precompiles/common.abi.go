...
// PackedEncodedSize returns the packed encoded size of Dec
func (t Dec) PackedEncodedSize() int {
	return 33
}

// PackedEncodeTo encodes Dec to packed ABI bytes in the provided buffer
func (value Dec) PackedEncodeTo(buf []byte) (int, error) {
	...
	// Field Precision: uint8
	n, err = abi.PackedEncodeUint8(value.Precision, buf[offset:])
	if err != nil {
		return 0, err
	}
	offset += n
...
// PackedEncode encodes Dec to packed ABI bytes
func (value Dec) PackedEncode() ([]byte, error) {
...
// PackedDecode decodes Dec from packed ABI bytes
func (t *Dec) PackedDecode(data []byte) (int, error) {
	if len(data) < 33 {
		return 0, io.ErrUnexpectedEOF
	}
	var err error
	...
	// Decode field Precision: uint8
	t.Precision, _, err = abi.PackedDecodeUint8(data[32:])
	if err != nil {
		return 0, err
	}
	return 33, nil
...
var _ abi.PackedTuple = (*Height)(nil)
...
// PackedEncodedSize returns the packed encoded size of Height
func (t Height) PackedEncodedSize() int {
	return 16
}

// PackedEncodeTo encodes Height to packed ABI bytes in the provided buffer
func (value Height) PackedEncodeTo(buf []byte) (int, error) {
	// Encode tuple fields sequentially (packed, no dynamic section)
	var (
		offset int
		n      int
	...
	// Field RevisionNumber: uint64
	n, err = abi.PackedEncodeUint64(value.RevisionNumber, buf[offset:])
	if err != nil {
		return 0, err
	}
	offset += n
	...
	// Field RevisionHeight: uint64
	n, err = abi.PackedEncodeUint64(value.RevisionHeight, buf[offset:])
	if err != nil {
		return 0, err
	}
	offset += n
...
// PackedEncode encodes Height to packed ABI bytes
func (value Height) PackedEncode() ([]byte, error) {
	buf := make([]byte, value.PackedEncodedSize())
	if _, err := value.PackedEncodeTo(buf); err != nil {
		return nil, err
	}
...
// PackedDecode decodes Height from packed ABI bytes
func (t *Height) PackedDecode(data []byte) (int, error) {
	if len(data) < 16 {
		return 0, io.ErrUnexpectedEOF
	}
	var err error
	// Decode field RevisionNumber: uint64
	t.RevisionNumber, _, err = abi.PackedDecodeUint64(data[0:])
	if err != nil {
		return 0, err
	}
	// Decode field RevisionHeight: uint64
	t.RevisionHeight, _, err = abi.PackedDecodeUint64(data[8:])
	if err != nil {
		return 0, err
	}
	return 16, nil
