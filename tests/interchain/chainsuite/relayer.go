...
// RestartRelayer stops and restarts the relayer
func (r *Relayer) RestartRelayer(ctx context.Context) error {
	rep := GetRelayerExecReporter(ctx)
	if err := r.StopRelayer(ctx, rep); err != nil {
		return err
	}
	return r.StartRelayer(ctx, rep)
}

// ConnectProviderConsumer creates the IBC connection and channel between provider and consumer chains
